...
import { filterSelectOptions, isOptionSelected, multipleValues, singleValue } from './options'
import { useRemoteSearch } from './useRemoteSearch'
import type { ZtSelectModelValue, ZtSelectOption, ZtSelectProps, ZtSelectValue } from './types'
import './select.scss'
...
const remoteSearch = useRemoteSearch(toRef(props, 'remoteMethod'), toRef(props, 'debounce'))
...
const selectedOptions = computed(() => selectedValues.value.flatMap(value => {
  const option = valueOptions.value.find(candidate => candidate.value === value)
...
const displayedOptions = computed(() => {
  if (props.remote) return props.remoteMethod
    ? hasRemoteSearch.value ? remoteSearch.options.value : props.options
    : []
  return props.filterable ? filterSelectOptions(props.options, keyword.value) : props.options
})
const navigableOptions = computed(() => visible.value
...
const activeOptionId = computed(() => {
  ...
  return option && !option.disabled ? optionId(activeIndex.value) : undefined
})
const dropdownStyle = computed(() => ({
  ...
  maxHeight: dropdownMaxHeight.value === undefined ? undefined : `${dropdownMaxHeight.value}px`,
}))
const inputValue = computed(() => searching.value ? keyword.value : selectedOption.value?.label ?? '')
const ariaInvalid = computed(() => formItem?.validateState.value === 'error' ? 'true' : undefined)
...
watch(() => overlay?.interactive.value, interactive => {
...
function setVisible(next: boolean) {
  ...
  if ((next && (effectiveDisabled.value || overlay?.interactive.value === false)) || visible.value === next) return
  visible.value = next
  if (next) {
...
function handleClick() {
  if (!searching.value && (props.filterable || props.remote)) comboboxElement.value?.select()
...
function selectOption(option: ZtSelectOption) {
  ...
  if (props.remote && !retainedRemoteOptions.value.some(candidate => candidate.value === option.value)) {
...
function handleFocusout(event: FocusEvent) {
  ...
  if (nextTarget instanceof Node
    && (rootElement.value?.contains(nextTarget) || listboxElement.value?.contains(nextTarget))) return
  emit('blur', event)
  close()
...
function isTagAction(target: EventTarget | null) {
  return target instanceof Element
    && target.closest('.zt-select__tag.is-custom, button, a, input, select, textarea, [role="button"], [contenteditable]')
}
...
function handleTagsWheel(event: WheelEvent) {
  ...
  const unit = event.deltaMode === 1 ? 16 : event.deltaMode === 2 ? tags.clientWidth : 1
  const nextScroll = Math.max(0, Math.min(maxScroll, tags.scrollLeft + (event.deltaX || event.deltaY) * unit))
...
function handleDocumentClick(event: MouseEvent) {
  ...
  if (rootElement.value?.contains(target) || listboxElement.value?.contains(target)) return
  close()
}
...
function updateDropdownPosition() {
  ...
  const footerHeight = dropdown.querySelector<HTMLElement>('.zt-select__footer')?.getBoundingClientRect().height ?? 0
  ...
  const menuHeight = list?.scrollHeight
    ? Math.min(280, list.scrollHeight) + chromeHeight
  ...
  listMaxHeight.value = Math.max(0, Math.min(280, availableHeight - chromeHeight))
...
function resolveDropdownZIndex() {
  const overlay = rootElement.value?.closest<HTMLElement>('.zt-modal, .zt-drawer')
  if (!overlay) return 2000
  const layer = Number.parseFloat(overlay.style.zIndex || getComputedStyle(overlay).zIndex)
...
<template>
  ...
  >
    <div ref="controlElement" class="zt-select__control">
      <span v-if="$slots.prefix" class="zt-select__prefix"><slot name="prefix" /></span>
      <span
    ...
    <Teleport to="body">
      <div
        ...
        :class="[
          { 'is-upward': placement === 'up', 'is-downward': placement === 'down' },
      ...
      >
        <div v-if="remote && remoteSearch.loading.value" class="zt-select__loading">
          <slot name="loading">加载中...</slot>
        </div>
        <div v-else-if="remote && remoteSearch.failed.value" class="zt-select__error">{{ remoteErrorText }}</div>
        <div
          v-else-if="displayedOptions.length"
          ref="optionsElement"
          class="zt-select__list"
          :style="{ maxHeight: listMaxHeight === undefined ? undefined : `${listMaxHeight}px` }"
        >
          <div
            ...
            :class="{ 'is-selected': isSelected(option), 'is-disabled': option.disabled, 'is-active': activeIndex === index }"
            role="option"
        ...
        <div v-if="$slots.footer" class="zt-select__footer"><slot name="footer" /></div>
      </div>
    </Teleport>
  </div>
</template>
