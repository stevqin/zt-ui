...
<template><section class="zt-result" :class="`zt-result--${status}`" role="status"><div class="zt-result__icon"><slot name="icon"><ZtIcon :name="iconName" /></slot></div><h2 v-if="title||$slots.title"><slot name="title">{{title}}</slot></h2><p v-if="subTitle||$slots['sub-title']"><slot name="sub-title">{{subTitle}}</slot></p><div v-if="$slots.extra" class="zt-result__extra"><slot name="extra" /></div></section></template>
