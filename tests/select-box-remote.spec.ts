...
describe('SelectBox batch confirmation', () => {
  it('keeps the panel open for review after a successful batch match, then commits separately', async () => {
    const w = box()
    await open(w); await enterPaste(w, '华东'); await click('.zt-select-box-panel__confirm')
    expect(w.emitted('update:modelValue')).toBeUndefined()
    expect(popup()).not.toBeNull()
    expect(rows().map(row => row.textContent)).toEqual(['华东'])
  ...
  it.each([false, true])('validates full matches then waits for final confirmation (remote=%s)', async remote => {
    ...
    if (remote) expect(remoteMethod).toHaveBeenLastCalledWith({ mode: 'batch', keywords: ['华东', '华南'] })
    expect(w.emitted('update:modelValue')).toBeUndefined()
    expect(popup()).not.toBeNull()
    expect(rows().map(row => row.textContent)).toEqual(['华东', '华南'])
    expect(document.querySelector('.zt-select-box-panel__pager')).toBeNull()
  ...
  it.each([false, true])('lists unmatched as warning then returns to all selected checkboxes (remote=%s)', async remote => {
  ...
  it('keeps batch loading independent from search results and suppresses duplicate clicks', async () => {
    ...
    expect(confirm.textContent).toContain('正在校验数据中')
    expect(confirm.disabled).toBe(true)
