...
<template>
  <section v-if="document" id="api" class="api-reference">
    ...
    <section
      v-for="owner in document.components"
      :key="owner.name"
      class="api-owner"
    >
      <h3>{{ owner.name }}</h3>
      <template v-for="section in sections" :key="section.key">
        <section
          v-if="owner[section.key].length"
          class="api-section"
          :data-api-section="section.key"
        >
          <div class="api-section__title">
            <h4>
              {{
                owner.service && section.key === 'props'
                  ? 'Options'
                  : section.title
              }}
            </h4>
          ...
          <ApiTable
            :rows="owner[section.key]"
            :owner="owner.name"
            :section="section.key"
            :show-default="section.key === 'props'"
          />
        </section>
      </template>
    </section>
    <section
      v-if="document.types.length"
      id="types"
      class="api-types"
    ...
    >
      ...
      <details
        v-for="type in document.types"
        :key="type.name"
        class="type-definition"
      >
        <summary>
          <code>{{ type.name }}</code
          ><small>{{ type.public ? '公共导出' : '依赖类型' }}</small>
        </summary>
        <pre><code>{{type.code}}</code></pre>
      </details>
    </section>
  </section>
</template>
