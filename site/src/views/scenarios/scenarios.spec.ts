...
describe('scenario interactions',()=>{
 ...
 it('does not change saved data when an edit is cancelled',async()=>{const w=render(OverlayEdit);await w.find('button').trigger('click');await settle();const input=document.querySelector<HTMLInputElement>('.zt-drawer-surface input')!;input.value='未保存修改';input.dispatchEvent(new Event('input',{bubbles:true}));await nextTick();const cancel=[...document.querySelectorAll<HTMLButtonElement>('.zt-drawer-surface button')].find(b=>b.textContent==='取消')!;cancel.click();await settle();expect(w.get('h3').text()).toBe('杭州湖滨店')})
