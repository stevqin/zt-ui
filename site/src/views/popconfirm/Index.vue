...
<template><div class="doc-section"><h1>Popconfirm 气泡确认框</h1><p>在当前操作附近完成二次确认，支持状态图标、说明和异步确认。</p><h2>危险操作</h2><DemoBlock :code="basicCode"><div class="confirm-row"><ZtPopconfirm title="确定删除这条记录？" description="删除后无法恢复" status="danger" @confirm="message='记录已删除'" @cancel="message='已取消'"><ZtButton status="danger">删除</ZtButton></ZtPopconfirm><ZtText>{{message}}</ZtText></div></DemoBlock><h2>异步确认</h2><DemoBlock :code="asyncCode" desc="Promise 完成前锁定按钮并显示进度；返回 false 或拒绝时保持打开。"><div class="confirm-row"><ZtPopconfirm title="提交审批？" :before-confirm="submit"><ZtButton :loading="saving">提交</ZtButton></ZtPopconfirm><ZtText status="info">{{message}}</ZtText></div></DemoBlock><h2>API</h2><p>beforeConfirm 可返回布尔值或 Promise；拒绝会触发 confirm-error，便于业务显示原因。</p><p><RouterLink to="/api/popconfirm">查看完整 Props、Events 和 Slots →</RouterLink></p></div></template>
