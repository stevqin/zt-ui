...
<template>
  <div class="doc-section form-doc">
    <h1>Form 表单</h1>
    <p>
      统一管理输入、选择、日期、验证码和附件字段的布局、校验状态与提交过程；FormGroup
      用于组织完整的业务表单。
    </p>
    <h2>全组件校验场景</h2>
    <DemoBlock
    ...
    <DemoBlock
      :code="Example02Code"
      desc="规则支持 blur、change、内置类型和公开校验方法；错误图标悬停或聚焦时显示 Tooltip。"
      ><Example02
    /></DemoBlock>
    <h2>标签布局</h2>
    <DemoBlock
    ...
    /></DemoBlock>
    <h2>行内表单</h2>
    <DemoBlock :code="Example04Code" desc="inline 适合筛选栏和紧凑操作区。"
      ><Example04
    /></DemoBlock>
    <h2>表单分组</h2>
    <DemoBlock
      :code="Example05Code"
      desc="ZtFormGroup 使用 fieldset/legend 组织区块，不改变字段校验规则。"
      ><Example05
    /></DemoBlock>
    <h2>异步自定义校验</h2>
    <DemoBlock
      :code="Example06Code"
      desc="validator 可返回 Promise、字符串、Error、boolean 或空值。"
      ><Example06
    /></DemoBlock>
    <h2>尺寸与整体禁用</h2>
    <DemoBlock
