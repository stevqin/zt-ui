...
<template>
  <div class="component-examples select-doc">
    <h3>基础单选与清空</h3>
    <DemoBlock
      :code="Example01Code"
      desc="使用 v-model 绑定单个值；clearable 允许快速清空当前城市。"
      ><Example01
    /></DemoBlock>
    ...
    <DemoBlock
      :code="Example03Code"
      desc="filterable 按选项标签做不区分大小写的本地匹配；选中、关闭或失焦后保留关键词，清空按钮会同时清空选择和关键词。"
      ><Example03
    /></DemoBlock>
    ...
    <DemoBlock
      :code="Example04Code"
      desc="查询固定等待 900 ms，便于观察加载；选中结果后保留远程搜索词，输入“失败”可稳定触发错误提示。"
      ><Example04
    /></DemoBlock>
    ...
    <DemoBlock
      :code="Example05Code"
      desc="prefix、option、selected、tag、empty、loading 和 footer 可承载品牌、区域与权限信息。"
      ><Example05
    /></DemoBlock>
    ...
    <DemoBlock
      :code="Example06Code"
      desc="disabled 可锁定整个选择器；单个选项也可按业务状态禁止选择。"
      ><Example06
    /></DemoBlock>
    ...
    <DemoBlock
      :code="Example07Code"
      desc="size 支持 mini、small、default、medium、large，适配不同信息密度。"
      ><Example07
    /></DemoBlock>
